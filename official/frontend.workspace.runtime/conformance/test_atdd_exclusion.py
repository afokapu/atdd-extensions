...
# A frontend source file with a silent-swallow catch — the detector flags this.
_DIRTY_SRC = (
...
def _build_polluted_root(base: Path) -> Path:
    ...
    normal = root / "src" / "application" / "submitVote.ts"
    ...
    twin = root / ".atdd" / "workspaces" / "x" / "fixtures" / "dirty" / "submitVote.ts"
...
def test_run_excludes_atdd_substrate_but_still_flags_normal_source(tmp_path: Path) -> None:
    ...
    assert any(v["file"].endswith("application/submitVote.ts") for v in res.violations)
