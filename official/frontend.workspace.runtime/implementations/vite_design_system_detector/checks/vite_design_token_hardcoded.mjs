#!/usr/bin/env node
// Detector: coder.vite.design-token-hardcoded  (disposition: strict)
//
// A hardcoded color literal — a `#rrggbb`/`#rgb` hex, an `rgb(...)`, or an
// `rgba(...)` — embedded in a `*.tsx` or `*.css` file forks the palette away from
// the centralized design tokens, so a future re-theme silently misses it. This
// detector flags each such literal (common neutrals tolerated).
//
...
const RULE_ID = "coder.vite.design-token-hardcoded";

const DEFAULT_EXCLUDES = ["node_modules", "dist", "build", ".next"];
// Colors are governed in both component source and stylesheets.
const SCAN_EXT = new Set([".tsx", ".css"]);
const TEST_RE = /\.(test|spec)\.[cm]?[jt]sx?$/;

// A six- or three-digit hex color, or an rgb()/rgba() function call.
const HEX_RE = /#([0-9a-fA-F]{6}|[0-9a-fA-F]{3})\b/g;
const RGB_RE = /\brgba?\s*\(/g;

// Common neutrals are tolerated (black/white/near-black), matching the core rule.
const NEUTRALS = new Set(["#fff", "#000", "#ffffff", "#000000", "#1a1a1a"]);
...
function scanFile(file, violations) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while ((m = HEX_RE.exec(line)) !== null) {
      if (NEUTRALS.has(m[0].toLowerCase())) continue;
      violations.push({
        rule_id: RULE_ID,
        file,
        line: i + 1,
        col: m.index + 1,
        evidence: `hardcoded color literal ${m[0]} instead of a design token`,
    ...
    while ((m = RGB_RE.exec(line)) !== null) {
      violations.push({
        ...
        line: i + 1,
        col: m.index + 1,
        evidence: `hardcoded ${m[0].replace(/\s*\($/, "")}() color literal instead of a design token`,
        source_line: line.trim(),
      });
    }
  }
...
function main() {
  ...
  process.stderr.write(
    `vite-detector(design-token-hardcoded): scanned ${roots.length} root(s), ${violations.length} violation(s)\n`,
  );
