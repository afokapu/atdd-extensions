#!/usr/bin/env node
// Detector: coder.vite.logging-silent-swallow  (disposition: suppress-and-clean)
//
// The Vite/React-stack realization of the agnostic "an exception handler must
...
const RULE_ID = "coder.vite.logging-silent-swallow";
...
function main() {
  ...
  process.stderr.write(
    `vite-detector(logging-silent-swallow): scanned ${roots.length} root(s), ${violations.length} violation(s)\n`,
  );
  process.exit(0);
}
