"""Discovery half of the frontend.workspace.runtime provider contract (contract_version 1.1.0).
...
— the SAME discover contract — because a frontend/JS runtime claims the same
...
def discover_implementations(
    ...
    for manifest_path in sorted(root.rglob(IMPLEMENTATION_GLOB)):
        ...
        if targets and targets != "frontend.workspace.runtime":
            continue
        try:
