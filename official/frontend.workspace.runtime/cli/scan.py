...
frontend.workspace.runtime provider.
...
INPUT
    ...
    env ATDD_IMPL_ID        implementation_id to resolve + run. Default
                            ``coder.vite.logging-silent-swallow``. ``--impl``
                            overrides.
...
DEFAULT_IMPL = "coder.vite.logging-silent-swallow"
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="CW-Phase 0 frontend.workspace.runtime provider CLI")
