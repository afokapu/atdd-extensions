...
- **id:** `atdd.workspace.dart-flutter` (scope segment `workspace`)
...
depends_on:
  workspaces:
    - id: atdd.workspace.dart-flutter
      contract: "^1.0.0"
```

Each `atdd.implementation.yaml` the extension ships declares
`targets_workspace: atdd.workspace.dart-flutter` and the `contract_version` it satisfies.
