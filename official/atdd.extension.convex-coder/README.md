...
`atdd.workspace.convex` provider (two-layer model). Targets that provider by id +
...
|---|---|---|---|---|
| `coder.convex.no-server-console-log` | 2 | strict | `atdd.workspace.convex` → `convex_no_server_console_log` | **active** (Phase 0 detector, proven) |
