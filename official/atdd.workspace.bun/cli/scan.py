#!/usr/bin/env python3
"""Provider CLI — the SUBPROCESS boundary between ATDD core and the
atdd.workspace.bun provider.
...
Sibling of ``atdd.workspace.python-pytest/cli/scan.py`` and
``frontend.workspace.runtime/cli/scan.py`` — the SAME CLI contract. The only
difference is what the provider runs: python-pytest collects a pytest report test;
this runtime runs the detector module directly via ``bun`` (the adapter's
``run.py``). All three cross the boundary as one JSON array of RAW v1.1 violations
on stdout, which is what makes the runtime interchangeable from core's side.
...
INPUT
    ...
    env ATDD_IMPL_ID        implementation_id to resolve + run. Default
                            ``htmx_hypermedia_detector``. ``--impl`` overrides.
...
DEFAULT_IMPL = "htmx_hypermedia_detector"
...
# not one hardcoded module. For this runtime the runnable and the detector are the
# same module (``report:`` == ``entrypoint:``); ``run.py`` executes it with ``bun``
# and reads back ATDD_VIOLATIONS_REPORT.
REPORT_FIELD = "report"
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="atdd.workspace.bun provider CLI")
