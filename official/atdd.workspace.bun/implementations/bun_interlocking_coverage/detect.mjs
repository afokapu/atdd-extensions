#!/usr/bin/env bun
// FAMILY validator: bun_interlocking_coverage
...
const checks = readdirSync(join(here, "checks"))
  ...
  .sort();
const td = mkdtempSync(join(tmpdir(), "atdd-bun-ilc-"));
const out = [];
...
process.stderr.write("family bun_interlocking_coverage: " + out.length + " violation(s)\n");
process.exit(0);
