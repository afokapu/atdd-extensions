#!/usr/bin/env bun
// Member check: coder.bun.complexity-nesting  (TypeScript metrics family — native Bun port)
//
...
const RULE = "coder.bun.complexity-nesting";
const LIMIT = M.MAX_NESTING;
...
for (const root of readRoots()) {
  for (const file of walk(root, excludes, M.TS_EXT)) {
    ...
    for (const f of M.extractFunctions(text)) {
      ...
      const value = M.nestingDepth(f.body);
      if (value <= LIMIT) continue;
      violations.push({
        rule_id: RULE, file, line: f.line, col: 0,
        evidence: `${f.name} nesting depth=${value} (>${LIMIT})`,
        source_line: (f.body.split("\n")[0] || "").trim(),
      });
    }
  }
}
process.stderr.write(`bun-metrics[nesting]: ${violations.length} violation(s)\n`);
emit(violations);
