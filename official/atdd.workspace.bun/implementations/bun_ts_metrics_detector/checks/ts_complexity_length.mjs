#!/usr/bin/env bun
// Member check: coder.bun.complexity-length  (TypeScript metrics family — native Bun port)
//
...
const RULE = "coder.bun.complexity-length";
const LIMIT = M.MAX_FUNCTION_LINES;
...
for (const root of readRoots()) {
  for (const file of walk(root, excludes, M.TS_EXT)) {
    ...
    for (const f of M.extractFunctions(text)) {
      ...
      const value = M.countCodeLines(f.body);
      if (value <= LIMIT) continue;
      violations.push({
        rule_id: RULE, file, line: f.line, col: 0,
        evidence: `${f.name} length=${value} (>${LIMIT})`,
        source_line: (f.body.split("\n")[0] || "").trim(),
      });
    }
  }
}
process.stderr.write(`bun-metrics[length]: ${violations.length} violation(s)\n`);
emit(violations);
