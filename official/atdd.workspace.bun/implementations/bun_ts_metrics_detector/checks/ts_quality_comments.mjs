#!/usr/bin/env bun
// Member check: coder.bun.quality-comments  (TypeScript metrics family — native Bun port)
//
...
const RULE = "coder.bun.quality-comments";
...
for (const root of readRoots()) {
  for (const file of walk(root, excludes, M.TS_EXT)) {
    ...
    if (!text) continue;
    const ratio = M.commentRatio(text);
    if (ratio >= M.MIN_COMMENT_RATIO) continue;
    violations.push({
      rule_id: RULE, file, line: 1, col: 0,
      evidence: `comment ratio ${(ratio * 100).toFixed(1)}% (< ${(M.MIN_COMMENT_RATIO * 100).toFixed(0)}%)`,
      source_line: (text.split("\n")[0] || "").trim(),
    });
  }
}
process.stderr.write(`bun-metrics[comments]: ${violations.length} violation(s)\n`);
emit(violations);
