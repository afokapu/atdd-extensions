"""Run half of the atdd.workspace.bun provider contract (contract_version 1.1.0).
...
contract. This is the Bun sibling of atdd.workspace.python-pytest/adapter/run.py
and frontend.workspace.runtime/adapter/run.py: it implements the SAME v1.1
contract (scan-mount env channel + structured JSON report), but the RUN command
shells out to Bun instead of pytest or node.

  * RUN COMMAND — ``bun <entrypoint>``. Bun executes ESM ``.mjs`` AND TypeScript
    ``.ts`` directly, with no build step, no transpile config and no
    ``node_modules``, so a detector can be authored in either and still run
    dependency-free. That is the whole reason this provider is a one-word delta
    from the node runtime: the CONTRACT is the env channel plus the report file,
    never the runner.

  * FULL-STACK NOTE — Bun is also this stack's server (``Bun.serve``), bundler and
    package manager, but none of that appears here. The provider's job is to run a
    detector and hand back RAW violations; the server/bundler/package-manager
    obligations are expressed as CONVENTIONS in atdd.extension.coder.htmx, not as
    provider behaviour. Keeping them out of the adapter is what lets this provider
    stay domain-agnostic and reusable by any Bun extension.
...
# Plain `bun` over the detector module. Bun runs .mjs and .ts natively, so unlike
# the node runtime there is no Phase 0.5 transpile step to graduate to: a detector
# that needs real TypeScript semantics can be authored as .ts TODAY and this same
# command runs it. Swapping to ("bun", "test", ...) for test-framework semantics
# would leave the contract untouched — the contract is the env channel + report
# file, not the runner.
RUN_COMMAND = ("bun",)
...
# Run-health exit codes the contract distinguishes. bun exits 0 on success and 1
...
def run_implementation(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="atdd-bun-report-") as tmp:
