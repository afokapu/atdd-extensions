"""Discovery half of the atdd.workspace.bun provider contract (contract_version 1.1.0).
...
and frontend.workspace.runtime/adapter/discover.py — the SAME discover contract —
because a Bun runtime claims the same discover/run protocol the engine speaks. The
ONLY difference between the three providers is the RUN command (``run.py``);
discovery is identical, which is exactly what makes the provider seam swappable.
...
def discover_implementations(
    ...
    for manifest_path in sorted(root.rglob(IMPLEMENTATION_GLOB)):
        ...
        if targets and targets != "atdd.workspace.bun":
            continue
        try:
