...
validator coder.duplication.no-intra-layer-code-python, emits RAW structured
...
Proves, per fixture:
  ...
  dirty -> provider RAW non-empty      -> disposition FAIL (strict, all unsuppressed)

Exit 0 only if every assertion holds. Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_duplication_python.py
"""
...
IMPL_DIR = IMPLS_ROOT / "duplication_detector"
EXPECTED_ID = "coder.duplication.no-intra-layer-code-python"

# Disposition registry the consumer builds from the migrated convention node.
...
def main() -> int:
    ...
    if (
        ...
        print("  PASS: copy-pasted same-layer fragment -> strict FAIL")
    else:
        print("  FAIL: expected non-empty raw of the dup rule_id -> FAIL")
        ok = False

    _section("RESULT")
    print("  ALL PASS — strict duplication (python) proven end-to-end"
