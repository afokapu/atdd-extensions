...
detector tester.smoke.no-collaborator-substitution (disposition suppress-and-clean,
...
(_consumer_disposition.py). The adapter and the stand-in never import each other;
only the RAW violation list crosses between.
...
Exit 0 only if every assertion holds. Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_no_collaborator_substitution.py
"""
...
IMPL_DIR = IMPLS_ROOT / "no_collaborator_substitution_detector"
EXPECTED_ID = "tester.smoke.no-collaborator-substitution"

# The disposition registry a real consumer would build from the CORE convention node.
...
def main() -> int:
    ...
    _section("RESULT")
    print("  ALL PASS — suppress-and-clean collaborator-substitution proof green; "
          "disposition cleanly separable" if ok else "  FAILURES ABOVE")
