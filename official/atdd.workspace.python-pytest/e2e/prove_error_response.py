"""End-to-end proof: the python-pytest provider discovers + runs the error-
response detector (STRICT coder.error-response.bare-string + coder.error-response
.code-format), emits RAW structured MULTI-RULE violations, and a SEPARATE
...
Proves, per fixture:
  ...
  dirty -> provider RAW = 2 over 2 rule_ids   -> disposition FAIL (both strict)

Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_error_response.py
"""
...
IMPL_DIR = IMPLS_ROOT / "error_response_detector"
EXPECTED_ID = "coder.error-response.bare-string"
RULE_BARE = "coder.error-response.bare-string"
RULE_CODE = "coder.error-response.code-format"

DISPOSITIONS = {RULE_BARE: "strict", RULE_CODE: "strict"}
...
def main() -> int:
    ...
    if (
        ...
        and dirty_rule_ids == {RULE_BARE, RULE_CODE}
        and not v_dirty.passed
        and len(v_dirty.unsuppressed) == 2
    ):
        print("  PASS: 2 raw (bare-string + code-format) -> strict FAIL, 2 unsuppressed")
    else:
        print("  FAIL: expected 2 raw over 2 rule_ids -> FAIL")
        ok = False

    _section("RESULT")
    print("  ALL PASS — strict error-response (multi-rule) proven end-to-end"
