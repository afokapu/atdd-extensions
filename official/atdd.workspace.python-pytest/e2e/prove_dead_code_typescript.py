...
validator coder.dead-code.reachability-typescript (disposition strict, severity
2) — a Python detector inspecting TypeScript source — emits RAW structured
violations, and a SEPARATE consumer-side disposition stand-in produces the
correct pass/fail.
...
violation FAILS; markers are ignored.

Proves, per fixture:
  clean -> provider RAW = []            -> disposition PASS
  dirty -> provider RAW = 1 (orphan.ts) -> disposition FAIL (1 unsuppressed)

Exit 0 only if every assertion holds. Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_dead_code_typescript.py
"""
...
IMPL_DIR = IMPLS_ROOT / "dead_code_typescript_detector"
EXPECTED_ID = "coder.dead-code.reachability-typescript"
...
def main() -> int:
    ...
    _section("3. fixtures/dirty  -> expect RAW=1 (orphan.ts) -> disposition FAIL")
    ...
    if (
        ...
        and dirty.violations[0]["file"] == "orphan.ts"
        and not v_dirty.passed
        and len(v_dirty.unsuppressed) == 1
    ):
        print("  PASS: 1 raw (orphan.ts) -> strict FAIL, 1 unsuppressed")
    else:
        print("  FAIL: expected 1 raw orphan -> strict FAIL")
        ok = False

    _section("RESULT")
    print("  ALL PASS — strict dead-code (typescript) proof green"
