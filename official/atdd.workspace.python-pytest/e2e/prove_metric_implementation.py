...
detector tester.acceptance-violation.metric-implementation-must-exist (disposition
strict, severity 4), emits RAW structured violations (declared signal.metric with
no backing compute()/passes() module), and a SEPARATE consumer-side disposition
stand-in — applied AFTER the run — produces the correct pass/fail.

This detector REALIZES a CORE obligation node (the obligation stays in core; this
is the python-pytest realization, like no_collaborator_substitution). Drives the
REAL provider adapter (../adapter/discover.py + run.py) — no stubs — and the REAL
consumer stand-in (_consumer_disposition.py). The adapter and the stand-in never
import each other; only the RAW violation list crosses between.

Proves, per fixture:
  clean -> provider RAW = []  -> strict verdict PASS
  dirty -> provider RAW = 2   -> strict verdict FAIL (both unsuppressed)

Exit 0 only if every assertion holds. Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_metric_implementation.py
"""
...
IMPL_DIR = IMPLS_ROOT / "metric_implementation_detector"
EXPECTED_ID = "tester.acceptance-violation.metric-implementation-must-exist"

# Strict per the CORE convention node; markers are ignored, every violation counts.
...
def _print_raw(res: run_mod.RunResult) -> None:
    ...
    for v in res.violations:
        print(f"    - [{v['rule_id']}] {v['file']}:{v['line']}  {v['evidence']}")
...
def main() -> int:
    ...
    _section("2. fixtures/clean  -> expect RAW=[] -> strict PASS")
    ...
    _section("3. fixtures/dirty  -> expect RAW=2 -> strict FAIL")
    ...
    if (
        ...
        print("  PASS: 2 raw missing-metric impls -> strict FAIL")
    else:
        print("  FAIL: expected 2 raw -> strict FAIL")
        ok = False

    _section("RESULT")
    print("  ALL PASS — strict metric-implementation proof green; disposition cleanly separable"
