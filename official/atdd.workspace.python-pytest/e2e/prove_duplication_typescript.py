...
validator coder.duplication.no-intra-layer-code-typescript, emits RAW structured
...
Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_duplication_typescript.py
"""
...
IMPL_DIR = IMPLS_ROOT / "duplication_detector_typescript"
EXPECTED_ID = "coder.duplication.no-intra-layer-code-typescript"
...
def main() -> int:
    ...
    if (
        ...
        and dirty_rule_ids == {EXPECTED_ID}
        and not v_dirty.passed
        and len(v_dirty.unsuppressed) == len(dirty.violations)
    ):
        print("  PASS: copy-pasted same-layer block -> strict FAIL")
    else:
        print("  FAIL: expected non-empty raw of the dup rule_id -> FAIL")
        ok = False

    _section("RESULT")
    print("  ALL PASS — strict duplication (typescript) proven end-to-end"
