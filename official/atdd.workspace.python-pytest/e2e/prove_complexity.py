...
MULTI-RULE python complexity validator (coder.refactor.complexity-cyclomatic +
-nesting + -length + -params + -cognitive — FIVE strict rule_ids from one run),
...
All five rule_ids are `strict`:
  ...
  dirty -> provider RAW = 6 over 5 rule_ids           -> disposition FAIL (6 unsuppressed)
          * cyclomatic: classify        (complexity > 10)
          * nesting:    deep_nest, cognitive_heavy (depth > 4)
          * length:     long_function    (LOC > 50)
          * params:     many_params      (params > 6)
          * cognitive:  cognitive_heavy  (cognitive complexity > 15)

Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_complexity.py
"""
...
IMPL_DIR = IMPLS_ROOT / "complexity_detector"
EXPECTED_ID = "coder.refactor.complexity-cyclomatic"
...
def main() -> int:
    ...
    _section("3. fixtures/dirty  -> expect RAW over all 5 rule_ids -> disposition FAIL")
    ...
    if (
        ...
        print(f"  PASS: {len(dirty.violations)} raw over 5 rule_ids -> FAIL, "
              f"{len(v_dirty.unsuppressed)} unsuppressed (all strict)")
    else:
        print("  FAIL: expected RAW over all 5 rule_ids -> strict FAIL")
        ok = False

    _section("RESULT")
    print("  ALL PASS — multi-rule python complexity (5 rule_ids) proven end-to-end"
