...
STRICT i18n-runtime validator (coder.presentation.i18n-config +
coder.presentation.i18n-switcher), emits RAW structured multi-rule violations, and
...
Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_i18n_runtime.py
"""
...
IMPL_DIR = IMPLS_ROOT / "i18n_runtime_detector"
EXPECTED_ID = "coder.presentation.i18n-config"

DISPOSITIONS = {
    "coder.presentation.i18n-config": "strict",
    "coder.presentation.i18n-switcher": "strict",
}
...
def main() -> int:
    ...
    if clean.structured and clean.violations == [] and v_clean.passed:
        print("  PASS: clean (config + switcher read the manifest) -> no raw -> PASS")
    else:
    ...
    if (
        ...
        and dirty_rule_ids == {"coder.presentation.i18n-config", "coder.presentation.i18n-switcher"}
        and not v_dirty.passed
        and len(v_dirty.unsuppressed) == 2
    ):
        print("  PASS: 2 raw (i18n-config + i18n-switcher) -> FAIL, 2 unsuppressed")
    else:
        print("  FAIL: expected 2 raw over 2 rule_ids -> strict FAIL")
        ok = False

    _section("RESULT")
    print("  ALL PASS — strict i18n-runtime validator proven end-to-end"
