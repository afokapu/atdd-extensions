...
detector tester.test-isolation.no-polluting-patterns (disposition strict,
...
is the python-pytest realization). Drives the REAL provider adapter
(../adapter/discover.py + run.py) — no stubs — and the REAL consumer stand-in
(_consumer_disposition.py). For a STRICT rule the consumer ignores markers
entirely: any violation -> FAIL.

Proves, per fixture:
  clean -> provider RAW = []          -> disposition PASS
  dirty -> provider RAW = 2 (bare-init-bad-cwd + core-bare-unscoped) -> FAIL

Exit 0 only if every assertion holds. Run:
    python3 official/atdd.workspace.python-pytest/e2e/prove_no_polluting_patterns.py
"""
...
IMPL_DIR = IMPLS_ROOT / "no_polluting_patterns_detector"
EXPECTED_ID = "tester.test-isolation.no-polluting-patterns"
...
def _print_raw(res: run_mod.RunResult) -> None:
    ...
    for v in res.violations:
        print(f"    - [{v['rule_id']}] {v['file']}:{v['line']}:{v['col']}  {v['evidence'][:60]}")
...
def main() -> int:
    ...
    _section("3. fixtures/dirty  -> expect RAW=2 -> strict disposition FAIL")
    ...
    if (
        ...
        print("  PASS: 2 raw -> strict FAIL, 2 unsuppressed")
    else:
        print("  FAIL: expected 2 raw -> strict FAIL")
        ok = False

    _section("RESULT")
    print("  ALL PASS — strict no-polluting-patterns proof green"
