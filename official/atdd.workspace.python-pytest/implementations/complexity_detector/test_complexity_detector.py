"""Runnable enforcement for coder.refactor.complexity-* (python, FIVE rule_ids).

Two layers (mirrors composition_completeness_detector / gsap_layer_detector):

  1. DETECTOR SELF-TESTS — pin the ported metric logic: the clean feature emits
     nothing; the dirty feature emits ALL FIVE rule_ids; each metric trips on its
     dedicated function; every record is the full v1.1 shape.
...
All five rule_ids are `strict`; the strict aggregation is the consumer's job
...
import complexity as detector  # noqa: E402
...
ALL_RULE_IDS = {
    ...
    detector.RULE_NEST,
    detector.RULE_LEN,
    detector.RULE_PARAMS,
    detector.RULE_COGNITIVE,
}
...
def test_dirty_fixture_emits_all_five_rule_ids() -> None:
...
def test_cyclomatic_trips_on_its_function() -> None:
    v = detector.scan_root(_HERE / "fixtures" / "dirty")
    hits = [x for x in v if x["rule_id"] == detector.RULE_CYCLO]
    assert any("classify" in x["evidence"] for x in hits)


def test_nesting_trips_on_its_function() -> None:
    v = detector.scan_root(_HERE / "fixtures" / "dirty")
    hits = [x for x in v if x["rule_id"] == detector.RULE_NEST]
    assert any("deep_nest" in x["evidence"] for x in hits)


def test_length_trips_on_its_function() -> None:
    v = detector.scan_root(_HERE / "fixtures" / "dirty")
    hits = [x for x in v if x["rule_id"] == detector.RULE_LEN]
    assert any("long_function" in x["evidence"] for x in hits)


def test_params_trips_on_its_function() -> None:
    v = detector.scan_root(_HERE / "fixtures" / "dirty")
    hits = [x for x in v if x["rule_id"] == detector.RULE_PARAMS]
    assert any("many_params" in x["evidence"] for x in hits)


def test_cognitive_trips_on_its_function() -> None:
    v = detector.scan_root(_HERE / "fixtures" / "dirty")
    hits = [x for x in v if x["rule_id"] == detector.RULE_COGNITIVE]
    assert any("cognitive_heavy" in x["evidence"] for x in hits)
...
def test_exclude_globs_drop_a_file() -> None:
    # Excluding the dirty module suppresses every violation (hermetic scope, §2).
    v = detector.scan_root(_HERE / "fixtures" / "dirty", exclude_globs=["complex_module.py"])
    assert v == []
...
def test_emit_raw_complexity_report() -> None:
    roots = _scan_roots()
