...
def test_resolve_match_trace_binds_declared_route():
    ...
    # BINDING BREAK (trace_to_declaration): asserts a route_id that is declared in NO interlocking
    # YAML — the trace does not bind the executed run back to a declared route.
    assert trace["route_id"] == "phantom-route-not-in-yaml"
