...
- **id:** `atdd.workspace.supabase` (scope segment `workspace`)
...
depends_on:
  workspaces:
    - id: atdd.workspace.supabase
      contract: "^1.0.0"
```

Each `atdd.implementation.yaml` the extension ships declares
`targets_workspace: atdd.workspace.supabase` and the `contract_version` it satisfies.
