...
- **id:** `atdd.workspace.typescript` (scope segment `workspace`)
...
depends_on:
  workspaces:
    - id: atdd.workspace.typescript
      contract: "^1.0.0"
```

Each `atdd.implementation.yaml` the extension ships declares
`targets_workspace: atdd.workspace.typescript` and the `contract_version` it satisfies.
