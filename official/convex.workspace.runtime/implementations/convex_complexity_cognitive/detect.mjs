#!/usr/bin/env node
// Detector: coder.convex.complexity-cognitive  (disposition: strict)
//
// Flags every Convex server function whose COGNITIVE COMPLEXITY (SonarQube /
// G. Ann Campbell algorithm) exceeds 15. Each control construct adds a flat +1
// structural increment plus a +N nesting increment (N = current depth) when it is
// nested inside another, so deeply nested if(if(if)) scores far higher than a flat
// chain; boolean / nullish operators add +1 each. Convex sibling of
// coder.refactor.complexity-cognitive (threshold 15).
//
...
function evaluate(fn) {
  const score = analyzeBlocks(fn).cognitive;
  if (score <= THRESHOLD) return null;
  return 'function "' + fn.name + '" has cognitive complexity ' + score + ' (threshold ' + THRESHOLD + ')';
}
...
const RULE_ID = "coder.convex.complexity-cognitive";
main();
