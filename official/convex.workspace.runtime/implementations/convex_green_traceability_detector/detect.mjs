#!/usr/bin/env node
// FAMILY validator: convex_green_traceability_detector
// Runs each member check (checks/*.mjs) as a subprocess and merges their RAW
// v1.1 reports into one. ONE implementation realizing a family of rule_ids (the
// Core pattern). The shared parser lives at the impl root (urn_header.mjs), NOT
// under checks/, so it is never spawned as a member.
...
const td = mkdtempSync(join(tmpdir(), "atdd-green-"));
const out = [];
...
process.stderr.write("family convex_green_traceability_detector: " + out.length + " violation(s)\n");
process.exit(0);
