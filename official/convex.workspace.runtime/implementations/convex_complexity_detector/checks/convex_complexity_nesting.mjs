#!/usr/bin/env node
// Detector: coder.convex.complexity-nesting  (disposition: strict)
//
// Flags every Convex server function whose MAXIMUM CONTROL-BLOCK NESTING DEPTH
// exceeds 4. Brace depth is tracked line by line; a level is counted only when a
// control-flow keyword (if / else / for / while / do / switch / try / catch /
// finally) opens the block, so object literals and arrow bodies do not inflate
// it. Convex sibling of coder.refactor.complexity-nesting-typescript (threshold 4).
//
...
function evaluate(fn) {
  const depth = analyzeBlocks(fn).maxNesting;
  if (depth <= THRESHOLD) return null;
  return 'function "' + fn.name + '" nests control blocks ' + depth + ' deep (threshold ' + THRESHOLD + ')';
}
...
const RULE_ID = "coder.convex.complexity-nesting";
main();
