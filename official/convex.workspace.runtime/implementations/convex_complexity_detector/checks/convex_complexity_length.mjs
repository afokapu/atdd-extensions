#!/usr/bin/env node
// Detector: coder.convex.complexity-length  (disposition: strict)
//
// Flags every Convex server function whose BODY LENGTH exceeds 50 lines of code,
// counting non-blank body lines and excluding blank lines and // or /* */ comment
// lines (no AST — regex + brace matching slice the body). Convex sibling of
// coder.refactor.complexity-length-typescript (threshold 50).
//
...
function evaluate(fn) {
  const loc = bodyLoc(fn);
  if (loc <= THRESHOLD) return null;
  return 'function "' + fn.name + '" body is ' + loc + ' lines of code (threshold ' + THRESHOLD + ')';
}
...
const RULE_ID = "coder.convex.complexity-length";
main();
