#!/usr/bin/env node
// Detector: coder.convex.complexity-cyclomatic  (disposition: strict)
//
// Flags every Convex server function whose CYCLOMATIC COMPLEXITY exceeds 10 —
// the count of independent paths, computed (no AST) as 1 + decision keywords
// (if / for / while / do / catch / case) + boolean & nullish operators
// (&& / || / ??) + ternaries. Trivial functions (< 3 code lines) are not scored.
// Convex sibling of coder.refactor.complexity-cyclomatic-typescript (threshold 10).
//
...
function evaluate(fn) {
  if (bodyLoc(fn) < 3) return null; // trivial functions are not scored
  const score = cyclomatic(fn);
  if (score <= THRESHOLD) return null;
  return 'function "' + fn.name + '" has cyclomatic complexity ' + score + ' (threshold ' + THRESHOLD + ')';
}
...
const RULE_ID = "coder.convex.complexity-cyclomatic";
main();
