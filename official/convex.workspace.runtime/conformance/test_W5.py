"""W5 GREEN / URN-traceability conformance (add-only, worker-scoped).

Scoped to W5's own new backend implementations — the GREEN traceability FAMILY
(`convex_green_traceability_detector`) and the no-globals SINGLETON
(`convex_green_no_globals`). For every declared rule_id: it must FIRE on its dirty
fixture and stay SILENT on its clean one; and each FAMILY, scanned over its whole
dirty tree at once, must emit its full declared set (the one-run-many-rules
contract). Mirrors the shared test_families.py logic but pinned to W5's dirs so it
neither depends on nor edits the wave-wide file."""
...
_W5_IMPLS = ["convex_green_traceability_detector", "convex_green_no_globals"]
